...
# NOTE: v0.26 introduced these per-step timeouts. On a slow embedding/LLM gateway
# (silra) GraphRAG legitimately takes hours-to-days to build a KB graph (verified:
# pre-v0.26 runs completed in 17-118h with NO timeout). The original 300s/chunk +
# 1800s caps kill every run at ~3h before it can finish. Raise the defaults and make
# them env-overridable so GraphRAG can run to completion like before.
DEFAULT_GRAPHRAG_BUILD_SUBGRAPH_TIMEOUT_PER_CHUNK_SECONDS = int(os.environ.get("GRAPHRAG_BUILD_SUBGRAPH_TIMEOUT_PER_CHUNK_SECONDS", 1800))
DEFAULT_GRAPHRAG_BUILD_SUBGRAPH_MIN_TIMEOUT_SECONDS = int(os.environ.get("GRAPHRAG_BUILD_SUBGRAPH_MIN_TIMEOUT_SECONDS", 3600))
DEFAULT_GRAPHRAG_MERGE_TIMEOUT_SECONDS = int(os.environ.get("GRAPHRAG_MERGE_TIMEOUT_SECONDS", 36000))
DEFAULT_GRAPHRAG_RESOLUTION_TIMEOUT_SECONDS = int(os.environ.get("GRAPHRAG_RESOLUTION_TIMEOUT_SECONDS", 36000))
DEFAULT_GRAPHRAG_COMMUNITY_TIMEOUT_SECONDS = int(os.environ.get("GRAPHRAG_COMMUNITY_TIMEOUT_SECONDS", 36000))
DEFAULT_GRAPHRAG_LOCK_ACQUIRE_TIMEOUT_SECONDS = int(os.environ.get("GRAPHRAG_LOCK_ACQUIRE_TIMEOUT_SECONDS", 3600))
