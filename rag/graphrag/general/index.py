...
async def generate_subgraph(
    ...
    from rag.graphrag.utils import _truncate_graph_to_fit
...
async def extract_community(
    ...
    for b in range(0, len(chunks), es_bulk_size):
        ...
        try:
            doc_store_result = await thread_pool_exec(settings.docStoreConn.insert, batch, search.index_name(tenant_id), kb_id)
            if doc_store_result:
