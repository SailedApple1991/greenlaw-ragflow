...
async def build_chunks(task, progress_callback):
    ...
    if task["parser_config"].get("auto_keywords", 0):
        ...
        chat_mdl = LLMBundle(task["tenant_id"], LLMType.CHAT, task["llm_id"], lang=task["language"])
...
async def run_dataflow(task: dict):
    ...
    if not any([re.match(r"q_[0-9]+_vec", k) for k in keys]):
        try:
            ...
            embedding_model = LLMBundle(task["tenant_id"], LLMType.EMBEDDING, embedding_id)
...
async def do_handle_task(task):
    ...
    try:
        ...
        embedding_model = LLMBundle(task_tenant_id, LLMType.EMBEDDING, task_embedding_id, lang=task_language)
    ...
    if task_type == "raptor":
        ...
        # bind LLM for raptor
        chat_model = LLMBundle(task_tenant_id, LLMType.CHAT, kb_task_llm_id, lang=task_language)
        # run RAPTOR
