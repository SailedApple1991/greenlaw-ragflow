...
async def async_chat(dialog, messages, stream=True, **kwargs):
    ...
    if stream:
        ...
        if full_answer:
            ...
            # Snapshot the full result for the cache before blanking the answer:
            # the streaming client already received the text as deltas, but the
            # cache needs the decorated answer to replay it on a later hit.
            cache_result = {**final}
            final["answer"] = ""
            yield final
    else:
