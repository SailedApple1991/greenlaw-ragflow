...
from api.apps.services.document_api_service import validate_document_update_fields, map_doc_keys, \
    ...
    reset_document_for_reparse, rename_doc_key, update_chunk_method_only
...
# --- fork custom: PUT update_document (kept alongside upstream's PATCH for frontend compat) ---
@manager.route("/datasets/<dataset_id>/documents/<document_id>", methods=["PUT"]) # noqa: F821
@login_required
@add_tenant_id_to_kwargs
async def update_document_put(tenant_id, dataset_id, document_id):
    """
    Update a document within a dataset.
    ---
    tags:
